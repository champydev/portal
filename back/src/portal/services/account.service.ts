...
export class AccountService {
  ...
  async refresh(autorizationHeader : string)
  {
    ...
    const requestPayload: JwtPayload = await <JwtPayload>jwt.verify(requestToken,this.jwtKey)
  ...
  async forgot(email : string)
  {
    ...
    if (user != null) {
      ...
      const token = await jwt.sign(payload, this.jwtKey, { expiresIn: "1d" });
      return token;
    }
    else{
  ...
  async createToken(user : User) {
    const payload: JwtPayload = { id:user.id,email:user.email,date: new Date()};    
    return await jwt.sign(payload, this.jwtKey, { expiresIn: 60 });
  }
